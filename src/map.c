...
typedef struct
{
	pair_object_s pair; // TODO could this be pair_u and allow differing keys/values?
	map_private_s *parent; // reference to parent gives compare function in entry compare
}
entry_s;
...
extern bool map_add(MAP ptr, const void *k, const void *v)
{
	...
	if (exists && map->overwrite)
	{
		e = (entry_s *)list_remove_item(map->entries, &e);
		if (map->free)
			free((void *)e->pair.p2);
	}
	else
	{
		e = calloc(sizeof( entry_s ), sizeof( byte_t ));
		e->parent = map;
		e->pair.p1 = (void *)k;
	}
	e->pair.p2 = (void *)v;
...
extern const void *map_get(MAP ptr, const void *k)
{
	...
	if (!list_contains(map->keys, k))
		return NULL;
	const entry_s *r = list_contains(map->entries, &((entry_s){ { (void *)k, NULL }, map }));
	return r->pair.p2;
}
...
extern const void *map_remove(MAP ptr, const void *k)
{
	...
	if (!list_contains(map->keys, k))
		return NULL;
	list_remove_item(map->keys, k);
	const entry_s *r = list_remove_item(map->entries, &((entry_s){ { (void *)k, NULL }, map }));
	if (map->free)
		free((void *)r->pair.p1);
